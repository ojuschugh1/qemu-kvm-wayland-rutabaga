...
create_vm_launcher() {
    ...
    cat > "$launcher" << EOF
#!/bin/bash
# QEMU VM with Wayland Forwarding Launcher
# Generated by wayland-vm-setup script
set -e
VM_NAME="$VM_NAME"
VM_DISK="$VM_DISK"
...
if [[ "\$INSTALL_MODE" == "true" ]] || [[ ! -s "\$VM_DISK" ]]; then
    if [[ -f "\$FEDORA_ISO" ]]; then
        BOOT_FROM_ISO="-cdrom \$FEDORA_ISO -boot d"
        print_status "Booting from ISO for installation"
    else
...
if [[ -n "\$WAYLAND_DISPLAY" ]] && [[ -S "\$XDG_RUNTIME_DIR/\$WAYLAND_DISPLAY" ]]; then
    print_status "Wayland detected on host: \$WAYLAND_DISPLAY"
    WAYLAND_ARGS="-display gtk,gl=on"
else
    print_warning "Wayland not detected, using X11 fallback"
    WAYLAND_ARGS="-display gtk,gl=on"
fi
if [[ ! -x "\$QEMU_BIN" ]]; then
    print_error "QEMU binary not found at: \$QEMU_BIN"
    print_error "Please ensure QEMU with rutabaga support is installed"
    exit 1
fi
if ! "\$QEMU_BIN" -device help | grep -q "virtio-gpu-rutabaga"; then
    print_error "QEMU does not have rutabaga support"
    print_error "Please build QEMU with rutabaga support first"
    exit 1
fi
print_status "Starting QEMU VM with rutabaga graphics..."
print_status "VM: \$VM_NAME"
print_status "Disk: \$VM_DISK"
print_status "RAM: $VM_RAM"
print_status "CPUs: $VM_CPUS"
if [[ "\$INSTALL_MODE" == "true" ]]; then
    print_status "Mode: Installation from ISO"
else
    print_status "Mode: Normal boot from disk"
fi
exec "\$QEMU_BIN" \
    -name "\$VM_NAME" \
    ...
    -enable-kvm \
    \
    \`# Graphics with rutabaga support\` \
    \$WAYLAND_ARGS \
...
main() {
    ...
    echo ""
    cd "$VM_DIR"
    # Always start in install mode after setup, since the disk may have just been recreated
    print_status "Starting VM in installation mode (ISO attached)."
    exec ./start-wayland-vm.sh --install
}
